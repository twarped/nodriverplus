    ...
    def do_GET(self):
        ...
        if immediate:
            try:
                ...
                logger.debug(
                    "client closed before immediate release (BrokenPipeError): %s",
                    self.path,
                )
            except ConnectionAbortedError:
                logger.debug(
                    "client closed before immediate release (ConnectionAbortedError): %s",
                    self.path,
                )
            except ConnectionResetError:
                logger.debug(
                    "client closed before immediate release (ConnectionResetError): %s",
                    self.path,
                )
            return
        ...
        try:
            ...
            logger.debug(
                "client closed before gated release (BrokenPipeError): %s", self.path
            )
        except ConnectionAbortedError:
            logger.debug(
                "client closed before gated release (ConnectionAbortedError): %s",
                self.path,
            )
        except ConnectionResetError:
            logger.debug(
                "client closed before gated release (ConnectionResetError): %s",
                self.path,
            )
...
class CloudflareSolver(NetworkWatcher):
    ...
    **TODO**:
                    ...
                    for target_id, gates in list(self.gates.items()):
                        ...
                        for request_id, path in list(gates.items()):
                            ...
                            if ts and now - ts > max_age:
                                logger.debug(
                                    "reaper releasing stale gate %s -> %s",
                                    request_id,
                                    path,
                                )
    ...
    async def on_detect(
        ...
        while (
            not tab._has_cf_clearance and not self._stop_reaper.is_set() and tab_alive()
        ):
    ...
    def should_ignore(self, url, method="get"):
        ...
        return (
            re.match(
                rf"^(blob:|data:|http://{address[0]}:{address[1]}/[\w]{{32}})", url
            )
            or method.lower() != "get"
        )
    ...
    async def on_request(self, tab, ev, extra_info):
        ...
        logger.debug(
            "gate %s -> %s (origin=%s)", ev.request_id, unique_url, ev.request.url
        )
    ...
    async def on_loading_failed(self, tab, ev, request_will_be_sent):
        ...
        if tab is None:
            return
        req_url = (
            request_will_be_sent.request.url if request_will_be_sent else "unknown"
        )
        logger.debug(
            "loading failed req=%s url=%s err=%s", ev.request_id, req_url, ev.error_text
        )
        ...
        try:
            ...
            logger.exception(
                "error releasing gate for failed request %s -> %s", ev.request_id, path
            )
        ...
        logger.debug(
            "releasing gate after loading failed %s <%s> err=%s",
            ev.request_id,
            path,
            ev.error_text,
        )
    ...
    async def on_response(self, tab, ev, extra_info):
        ...
        if self.should_ignore(ev.response.url) or tab is None:
            ...
            return
        headers = (
            extra_info.headers.to_json()
            if extra_info
            else ev.response.headers.to_json()
        )

        cf_ray = headers.get("cf-ray")
        cf_chl_gen = headers.get("cf-chl-gen")
        set_cookie = headers.get("set-cookie", "")
        cookies: list[cdp.network.Cookie] = await tab.send(
            cdp.network.get_cookies([tab.url])
        )
        ...
        if cf_chl_gen:
            ...
            try:
                ...
                logger.debug(
                    "no gate for response %s %s", ev.request_id, ev.response.url
                )
                return
            self.server.release_block(path)
